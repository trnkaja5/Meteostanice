...
\ctuifswitch { savetoner } {
	...
	\ctuifswitch { monochrome } {
		...
		\definecolor { Automatizace } { RGB }{ 53, 170, 71 }
...
%	}
	\DeclareDocumentCommand\chapter{ s o m }{\relax
		...
		\hbox_to_wd:nn \linewidth {
			% In the first column its right-aligned, in the second left-aligned.
			% It can't be more than one line.
			\leavevmode\fontsize{18}{18}\selectfont\strut\bfseries\sffamily##3
			\if@firstcolumn\else\hfil\fi
		}\par
		\skip_vertical:n { 2pt }
		\everypar{\noindent}
...
\cs_new:Nx \ctuthesis_geometry_hmarginratio: {5\string :6}
...
\ctuifswitch { layout-short } {
	...
	\dim_const:Nn \g_ctuthesis_geometry_textheight_dim { 0.84\paperheight }
	\cs_new:Nx \ctuthesis_geometry_vmarginratio: {12\string :6}
}
...
\long\def\ctu@title@boxify#1#2#3#4{
	...
	\noindent\begin{tabular}{@{}l@{\hspace*{\ctu@title@rulesep}}l@{}}
		\color{Automatizace}\rule[-\dp\ctu@title@box]{\ctu@title@rulewidth}{\dimexpr\ht\ctu@title@box+\dp\ctu@title@box}
	&
		\box\ctu@title@box
...
\fancypagestyle{twocol}{
	...
	\fancyhead [ C ] {
		...
		\smash {
			\color{Automatizace}
			\rule [ \dimexpr - \textheight - \headsep - 6pt ]{ 12pt }{ \textheight +26pt } 
		}
	}
...
\fancypagestyle { headings } {
	...
	\fancyhead [ LE ] { \ctuthesis_fancy_xlap:nn { right } { \sf\leftmark \ctuthesis_fancy_leaders:n { l } } }

...
\ctutemplateset{titlepage}{
	...
	\vss%
	{\noindent\leavevmode\color{Automatizace}\vrule width 12pt\hspace*{6pt}}
	...
	\vbox to \dimexpr\textheight + \baselineskip + \headsep + \footskip{
		...
		%
		% The document type
		{\fontsize{12}{12}\selectfont \ctufield[title]{doctype}
		\par}
		%
		\vspace*{2ex}
		% A table containing: the CTU logo and name, faculty number, faculty and deparment name
		\begin{tabular}{@{}l@{\hspace*{1em}}l@{}}
			\large\includegraphics[height=\dimexpr4\normalbaselineskip-1ex]{sps-logo.png}
		\end{tabular}\par
		%
		\vspace*{1ex}
		%
		{\fontsize{14}{14}\selectfont 26-41-M/01 Elektrotechnika \par}
		%
		\vspace*{30ex}
		%
		% The title, in two languages
		{\fontsize{28}{28}\selectfont \ctufield[title]{title}
		\par}
		%
		\vspace*{7ex}
		%
		% The author
		{\fontsize{16}{16}\selectfont \ctufield{author}
		\par}
		%
		...
		\ctuiffield*[title]{field}{\fieldofstudyname :~Mechatronika\par}{}
		\ctufield{year}\par
		\vspace*{5mm}
		}
	}
